...
import {Positions, ZoneMPSN,RLS1, RLS2, RLS3} from './PositionsA';
...
const LayersA = (props) => { 
	...
	const mapE = useMapEvents({		
	 	zoomend: () => {		  
	 	  	localStorage.setItem('zoomA', mapE.getZoom());		  
	 	},
	 	moveend: () => {						
	 		localStorage.setItem('centerA', JSON.stringify([mapE.getCenter().lat,mapE.getCenter().lng]));
	 	},
		overlayadd: (e)=>{
	...
	return (
    	<>
      		<LayersControl  position="topright">          		
				...
				<LayersControl.Overlay checked  name={"<span class='span_control rls2'>РЛС1</span>"} >
					<RLS1/>					
				</LayersControl.Overlay>
				<LayersControl.Overlay  checked name={"<span class='span_control rls2'>РЛС2</span>"} >
					<RLS2/>					
				</LayersControl.Overlay>
				<LayersControl.Overlay  checked name={"<span class='span_control rls2'>РЛС3</span>"} >
					<RLS3/>					
				</LayersControl.Overlay>				
			</LayersControl>
